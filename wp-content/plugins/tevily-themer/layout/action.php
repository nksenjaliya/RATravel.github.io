...
class GVA_Layout_Action extends GVA_Layout_Model{
   ...
   public function ajax_auth_init(){ 
      add_action( 'wp_ajax_tevily_add_template_layout', array($this, 'add_template_layout') );
      add_action( 'wp_ajax_tevily_duplicate_template_layout', array($this, 'duplicate_template_layout') );
      add_action( 'wp_ajax_tevily_set_header_footer_layout', array($this, 'set_header_footer_layout') );
      add_action( 'wp_ajax_tevily_set_state_config', array($this, 'set_state_config') );
      add_action( 'wp_ajax_tevily_delete_template_layout', array($this, 'delete_template_layout') );
   }

   public function add_template_layout(){
      check_ajax_referer('tevily_ajax_security_nonce', 'security');
      ...
      echo json_encode(
         array(
            'status'  => 'success',
            'message' => esc_html__( $name . " created", 'tevily-themer')
         )
      );
      die();
   }
   ...
   public function set_header_footer_layout(){
      ...
      echo json_encode(
         array(
            'status'  => 'success',
            'message' => esc_html__( "Changes saved successfully", 'tevily-themer')
         )
      );
      die();
   }
   ...
   public function set_state_config(){
      ...
      if( empty($_POST['id']) || empty($_POST['type']) ){ 
         echo json_encode(
            array(
               'status'  => 'success',
               'message' => esc_html__( "Changes Unsuccess", 'tevily-themer')
            )
         );
         return;
      }
   ...
   public function duplicate_template_layout(){
      ...
      if (is_wp_error($new_post_id)) {
         echo json_encode(
            array(
               'status'  => 'success',
               'message' => esc_html__( "Can't Duplicate", 'tevily-themer')
            )
         );
         die();
      }
      ...
      echo json_encode(
         array(
            'status'  => 'success',
            'message' => esc_html__( "Duplicate success", 'tevily-themer')
         )
      );
      die();
   ...
   function delete_template_layout(){
      ...
      if($template_type){
         ...
         if( count($get_templates) < 2){
            echo json_encode(
               array(
                  'status'  => 'unsuccess',
                  'message' => esc_html__( "Can't delete when have only one item", 'tevily-themer')
               )
            );
            die();
         }
      ...
      global $wpdb;
      $meta_key = 'tevily_template_layout';
      ...
      if( $count->THE_COUNT > 0 ){
         echo json_encode(
            array(
               'status'  => 'unsuccess',
               'message' => esc_html__( "Can't delete, some content use this template layout!", 'tevily-themer')
            )
         );
         die();
      }
      ...
      if (!$post) {
         echo json_encode(
            array(
               'status'  => 'unsuccess',
               'message' => esc_html__( "Can't Delete Template", 'tevily-themer')
            )
         );
         die();
      }
      ...
      echo json_encode(
         array(
            'status'  => 'success',
            'message' => esc_html__( "Template deleted", 'tevily-themer')
         )
      );
      die();
