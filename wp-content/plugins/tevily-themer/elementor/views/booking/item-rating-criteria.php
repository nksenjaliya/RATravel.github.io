<?php
	...
	if(!$total_rating){
		echo '<div class="tevily-no-rating">'.esc_html__('No reviews yet', 'tevily-themer').'</div>';
		return;
	}
	...
	if ($total_rating <= $step) {
		$text = esc_html__('Bad', 'tevily-themer');
	}elseif ($total_rating > $step && $step * 2 >= $total_rating) {
		$text = esc_html__('Not Bad', 'tevily-themer');
	}elseif ($total_rating > $step * 2 && $step * 3 >= $total_rating) {
		$text = esc_html__('Good', 'tevily-themer');
	}elseif ($total_rating > $step * 3 && $step * 4 >= $total_rating) {
		$text = esc_html__('Very Good', 'tevily-themer');
	}elseif ($total_rating > $step * 4) {
		$text = esc_html__('Wonderful', 'tevily-themer');
	}
?>
...
<div class="tevily-single-rating-criteria">
	<div class="box-content">
		...
		<div class="rating-value">
			...
			<div class="vote-number"><?php printf('%s ' . _n('verified review', 'verified reviews', $total_votes, 'tevily-themer'), $total_votes); ?></div>
		</div>
