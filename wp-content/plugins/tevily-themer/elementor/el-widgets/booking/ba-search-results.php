...
class GVAElement_BA_Search_Results extends GVAElement_Base{
	...
	public function get_title() {
		return __('BA Search Results', 'tevily-themer');
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			array(
				...
				'description'	=> esc_html__('How much items per page to show', 'tevily-thmer'),
				'default' => '12',
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'default' => 'price',
				'options' => [
					'title'      	=> esc_html__('Title', 'tevily-themer'),
					'price'  		=> esc_html__('Price', 'tevily-themer'),
		...
		$this->add_control(
			'layout_heading',
			array(
				'label'   => esc_html__( 'Layout __________', 'tevily-themer' ),
		...
		$this->add_control(
			...
			array(
				...
				'options' => [
					...
					'list'  		=> esc_html__('List', 'tevily-themer'),
				]
			)
		);
	...
	protected function render() {
		...
		printf( '<div class="gva-element-%s gva-element">', $this->get_name() );
	            ...
	            <div class="babe_search_results_filters">
	                  ...
	                  <div class="count-posts">
	                     <strong class="count"><?php echo esc_html($results['posts_count']); ?></strong>&nbsp;
									<?php echo (1 < $results['posts_count']) ? esc_html__('Tours', 'tevily-themer') : esc_html__( 'Tour', 'tevily-themer' ); ?>
	                     </div>
						<?php } ?>

						<?php if(isset($results['sort_by_filter']) && !empty($results['sort_by_filter'])){
								printf('<div class="filter-sort"><span>' . esc_html__('Sort by', 'tevily-themer') . '</span>%s</div>', $results['sort_by_filter']);
						} ?>
	            </div>
			...
			<?php }else{
				echo '<h2 class="empty-list">' . esc_html__( 'No available tours', 'tevily-themer' ) . '</h2>';
				echo '<p>' . esc_html__( 'It seems we can’t find what you’re looking for. ', 'tevily-themer' ) . '</p>';
			}
		print '</div>';
	}
