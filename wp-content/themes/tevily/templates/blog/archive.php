		  	...
		  	<?php 
		  		if(have_posts()){
		  			echo '<div class="posts-grids blog-grid-style">';
		  				echo '<div class="post-items post-masonry-style ' . esc_attr($classes_post_grid) . '">';
					...
					echo '<div class="pagination">';
						 echo tevily_pagination();
					echo '</div>';
			 	}else{
				 	echo '<div class="search-no-results-content">';
			  			echo '<div class="message">';
			  				echo esc_html__( 'Sorry, but nothing matched your search terms. Please try again with some different keywords.', 'tevily' );
			  			echo '</div>';
			  		echo '</div>';
		  		 	get_search_form();
			 	}
